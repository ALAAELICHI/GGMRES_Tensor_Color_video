function [xapprox,Errel,Res,volume,TotIt,stopReSt] = ReStart_GMRES(AA,BB,b,sigma,eta,optn,ne,ni,x)
%
...
%                   given volume, with precoditioning
%                 - optn=6: projection (nonnegativity & volume), but no
%                   preconditioning
%                 - optn=7: standard restarted GMRES
...
for extit=1:ne
    ...
    [xapprox_temp,Res_temp,Errel_temp,stopReSt_temp]=GMRES_StdGS(AA,BB,b,ni,s,sigma,eta,x0,x);
    EfIt=stopReSt_temp;
    if psf
        ...
        switch optn
            ...
            case 7
                M=spdiags(ones(N,1),0:0,N,N);
        end
    end    
    EfIt=stopReSt_temp;
    Errel((TotIt+1):(TotIt+EfIt))=Errel_temp(1:EfIt);
    Res((TotIt+1):(TotIt+EfIt))=Res_temp(1:EfIt);
